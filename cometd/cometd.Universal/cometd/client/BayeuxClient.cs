...
namespace Cometd.Client
{
    ...
    public class BayeuxClient : AbstractClientSession, IBayeux
    {
        ...
        private bool scheduleAction(TimerCallback action, long interval, long backoff)
        {
            // @@ax: What about support for multiple timers?
            long wait = interval + backoff;
            if (wait <= 0) wait = 1;
            var timer = new Timer(action, null, (int)wait, Timeout.Infinite);
